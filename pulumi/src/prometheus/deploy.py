...
def deploy_prometheus(
    ...
    if version is None:
        ...
        pulumi.log.info(
            f"Setting helm release version to latest stable: {chart_name}/{version}"
        )
    else:
    ...
    service_grafana = k8s.core.v1.Service(
        "service-grafana",
        metadata=k8s.meta.v1.ObjectMetaArgs(name="grafana", namespace=ns_name),
        spec={
            "type": "ClusterIP",
            "ports": [
    ...
    service_prometheus = k8s.core.v1.Service(
        "service-prometheus",
        metadata=k8s.meta.v1.ObjectMetaArgs(name="prometheus", namespace="monitoring"),
        spec={
            "type": "ClusterIP",
            "ports": [
                {
