...
def deploy_openunison(
        ...
        kubernetes_dashboard_release: Chart
    ):
    ...
    openunison_certificate = CustomResource(
        ...
        spec={
            ...
            "dnsNames": [f"*.{domain_suffix}", domain_suffix],
            "issuerRef": {
                "name": cluster_issuer,
                "kind": "ClusterIssuer",
    ...
    cluster_issuer_object = k8s_client.CustomObjectsApi().get_cluster_custom_object(
        group="cert-manager.io",
        version="v1",
    ...
    pulumi.log.debug("Loading CA from {}".format(cluster_issuer_ca_secret_name))

    ca_secret = k8s_client.CoreV1Api().read_namespaced_secret(
    ...
    pulumi.log.debug("CA Certificate {}".format(ca_cert))

    deploy_openunison_charts(
        ca_cert=ca_cert,
        ...
        openunison_certificate=openunison_certificate,
        config=k8s_provider.config,
        kubernetes_dashboard_release=kubernetes_dashboard_release
    )


def deploy_openunison_charts(
        ca_cert,k8s_provider: k8s.Provider,
        kubernetes_distribution: str,
        project_name: str,
        namespace: str,
        domain_suffix: str,
        openunison_certificate,
        config,
    ...
    openunison_helm_values = {
        ...
        "dashboard": {
            ...
            #"service_name": kubernetes_dashboard_release.name.apply(lambda name: "kubernetes-dashboard-" + name)   ,
    ...
    orchesrta_login_portal_helm_values["dashboard"]["service_name"] = kubernetes_dashboard_release.name.apply(lambda name: name)
