...
Pulumi with Google Cloud is a fun combo. The credential security story is def important, there are a couple of ways to go about it.

**Option 1: Pulumi Cloud and Google Cloud OIDC Integration**

The recommended way is to set up the [Pulumi Cloud Environments Secrets and Configuration](https://www.pulumi.com/docs/esc/) (ESC) feature with Google Cloud OIDC integration. This integration issues temporary credentials for the duration of your Pulumi ESC session. You then proceed with consuming the credentials in Pulumi Programs, or on the CLI with other tools like the GCP CLI.

**Option 2: Using Environment Variables**

Alternatively, you can save the contents of your `application_default_credentials.json` to Pulumi ESC. Here's a snippet as an example:

1. In your Pulumi Environment configuration, define the following:

```yaml
values:
  files:
    GOOGLE_APPLICATION_CREDENTIALS: ${gcp.application_default_credentials}
    KUBECONFIG: ${kubernetes.kubeconfig}

  gcp:
    application_default_credentials: >
      {"json":"string", "gcp":"go", "credentials":"here"}

  kubernetes:
    kubeconfig: >
      {"kind": "Config", "apiVersion": "v1", "preferences": {}, "clusters": [{"name": "kube-cluster", "cluster": {"server": "https://127.0.0.1:16443", "certificate-authority-data": "...trunc..."}}], "users": [{"name": "admin", "user": {"client-certificate-data": "...trunc...", "client-key-data": "..trunc..."}}], "contexts": [{"name": "cube", "context": {"cluster": "kube-cluster", "user": "admin"}}], "current-context": "cube"}
```

2. When you need the secret locally for cli auth, or during ci pipeline runs you can access the secret via the command:

```bash
eval $(esc open YOUR_ENVIRONMENT_NAME --format shell)
```

This will create 0600 mode `/tmp/esc-123412341234` files containing the secret contents you defined in Pulumi ESC. It will also export the key half of the .values.files key:value pair as an environment variable pointing to your /tmp/esc-XXX file.

Then, you're all set to run any GCP CLI commands, Pulumi commands, or any other operations that require these credentials. They will use the temporary credential file for the session.

If you want an easy way to try it all out without the hastle of installing dependencies and such, you can launch a Github Codespaces from our iac developer devcontainer and template repo here: [github.com/pulumi/devcontainer](https://github.com/pulumi/devcontainer)